...
import {CertTableBodyCell} from './CertTableBodyCell';
...
export const CertTableBodyRow: FC<Props<any>> = ({row, rowIndex, table}) => {
      ...
      title={
        <>
          {rowData.entity.info ? (
            <Box>
              <CertSummary
                firstSeen={rowData.entity.firstSeen}
                currentCert={certHistory.actual}
                previousCert={certHistory.prev}
                totalRecertifications={rowData.entity.totalRecertifications}
                relativeTime={true}
              />
            </Box>
          ) : (
            <i>
              Something strange happened, there is no row data to render the
              tooltip. Please speak with developers
            </i>
          )}
        </>
      }
